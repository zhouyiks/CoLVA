...
def qwen2vl_collate_fn(instances: Sequence[Dict],
    ...
    for k, v in first.items():
        if k in ('image_grid_thw', 'image_flags', 'num_images', 'ot_pixel_values', 'ot_visual_prompts'):
        ...
        if k in ('num_vprompts', 'has_visual_prompt'):
            extend_list = []
            for example in instances:
        ...
        if k in ('visual_prompts', ):
            k_list = [example[k] for example in instances]
            data_dict[k] = k_list
