...
def internvl_collate_fn(instances: Sequence[Dict],
    ...
    has_vprompt = any(inst.get('visual_prompts') is not None for inst in instances)
    if use_varlen_attn:
    ...
    if has_vprompt:
        visual_prompts = torch.cat([vprompts.flatten(0, 1) for vprompts in visual_prompts])
    ...
    for k, v in first.items():
        if k not in ('image_flags', 'num_patches', 'num_vprompts', 'sampled_mark_token_ids', 
                     'pixel_values', 'visual_prompts', 'merged_visual_prompts') and v is not None and not isinstance(v, str):
