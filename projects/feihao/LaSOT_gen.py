...
final_json_data = {
    "task": "video object tracking, LaSOT long video",
    "data_source": "LaSOT",
    "type": "comprehension",
    "modality": {
        "in": ["image", "text"],
...
split_data_list = {'person': [], "animal": [], "vehicle": [], "others": []}
split_key_str = {
    'person': ["person"],
    'animal': ["cat", "crocodile", "horse", "lion", "mouse", "sheep", "turtle", "cattle", "elephant", "gecko", "kangaroo",
               "lizard", "bear", "chameleon", "deer", "giraffe", "pig", "spider", "tiger", "dog", "fox", "goldfish",
               "leopard", "monkey", "sepia", "squirrel", "bird", "crab", "frog", "gorilla", "hippo", "rabbit", "shark", "zebra"],
    'vehicle': ["airplane", "boat", "tank", "bicycle", "bus", "train", "car", "motorcycle", "truck", "racing"],
    'others': ["electricfan", "gametarget", "guitar", "swing", "basketball", "book", "robot", "skateboard", "umbrella", "bottle", "flag", "hat",
               "kite", "microphone", "rubicCube", "volleyball", "coin", "helmet", "pool", "yoyo", "drone", "surfboard"],
        ...
        if _split is None:
            for _key_str in split_key_str['animal']:
                if _key_str in category_name:
                    ...
                    break
        if _split is None:
            for _key_str in split_key_str['vehicle']:
                if _key_str in category_name:
                    _split = 'vehicle'
                    break
        if _split is None:
            for _key_str in split_key_str['others']:
                if _key_str in category_name:
                    _split = 'others'
                    break
        if _split is None:
