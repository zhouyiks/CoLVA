...
def llava_collate_fn(instances: Sequence[Dict],
    ...
    for k, v in first.items():
        if k in ('image_flags', 'image_sizes', 'num_images', 'visual_prompts', 'ot_pixel_values', 'ot_visual_prompts'):
        ...
        if k in ('num_vprompts', 'num_patches'):
            extend_list = []
            for example in instances:
